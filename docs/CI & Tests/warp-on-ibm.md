...
Currently, all secrets must be passed to every job since GitHub Actions does not provide a straightforward way to selectively pass individual secrets to reusable workflows (e.g. - it is not possible to pass only VM_CONFIG, since then all other inherited secrets are no longer passed to the workflow)

**Limitation Details**:
- GitHub Actions reusable workflows require all secrets to be explicitly passed from the calling workflow.
- This means that even if a reusable workflow only needs one secret, all related secrets must be passed through the dispatcher.
- Each secret must be individually declared in both the dispatcher workflow (`secrets:` section) and the reusable workflow (`secrets:` input section).
